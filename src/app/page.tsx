...
const STEPS: [string, string][] = [
    ['1. Start a run', 'Point it at a product repo — here, or from a Claude Code session with /new-run.'],
    [
        '2. Agent generates',
        'The agent extracts the brand kit, drafts a brief and themes, then composes copy and creatives.',
    ],
    ['3. You review', 'Approve or flag everything on the run page. The cockpit reviews; the agent generates.'],
]
        ...
        <div className='space-y-10'>
            ...
            <ol className='grid gap-4 sm:grid-cols-3'>
                {STEPS.map(([title, detail]) => (
                    <li key={title} className='rounded-lg bg-neutral-900 p-4'>
                    ...
                    <ul className='divide-y divide-neutral-800 rounded-lg bg-neutral-900'>
                        {runs.map((run) => (
                            <li key={run.id}>
                                <Link
                                    href={`/runs/${run.id}`}
                                    className='flex items-center justify-between gap-4 px-5 py-3 hover:bg-neutral-800'
                                >
                                    <span className='font-mono text-sm'>{run.id}</span>
                                    <span className='truncate text-sm text-neutral-400'>{run.repoPath}</span>
                                    <span
                                        className={`shrink-0 rounded px-2 py-1 text-xs ${
                                            STATUS_LABELS[run.status].actor === 'you'
